...
int create_user(char *name, char *phone, char *password) {
    ...
    if ((fd = open("users.bin", O_RDWR | O_CREAT | O_APPEND, 0644)) == -1) {
...
int authenticate_admin(char *name, char *password){
    return authenticate(name, password, "admins.bin");
}

int authenticate_user(char *name, char *password){
    return authenticate(name, password, "users.bin");
}
